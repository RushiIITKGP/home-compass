...
export function FitBreakdown({ recommendations }: { recommendations: Recommendation[] }) {
  ...
  return (
    <details className="mt-1 w-full rounded-lg border border-rule bg-card-dim px-4 py-2.5 text-sm">
      <summary className="cursor-pointer select-none font-mono text-xs uppercase tracking-wide text-muted hover:text-ink">
        ...
        {CRITERIA.map((criterion) => {
          ...
          return (
            <div key={criterion.key} className="flex items-baseline justify-between gap-3">
              <span className={applies ? "text-ink" : "text-muted"}>
              ...
              <span className="font-mono text-xs tabular-nums text-brass shrink-0">
                {criterion.weight}%
              </span>
            </div>
          );
