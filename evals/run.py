...
def run_confidence_suite() -> list[RunResult]:
    ...
    for case in CONFIDENCE_CASES:
        ...
        problems = []
        if not math.isclose(score, case["expected_score"], abs_tol=1e-9):
...
def _field_matches(field: str, expected, actual) -> bool:
    ...
    if isinstance(expected, (int, float)) and isinstance(actual, (int, float)):
        return math.isclose(expected, actual, abs_tol=1e-6)
...
def run_routing_suite(fast_llm, smart_llm, runs: int) -> list[RunResult]:
    from langgraph.checkpoint.memory import MemorySaver

    results = []
    for case in ROUTING_CASES:
        for run in range(1, runs + 1):
            graph = build_graph(fast_llm, smart_llm, FAKE_TOOLS, checkpointer=MemorySaver())
...
def run_groundedness_suite(llm, runs: int) -> list[RunResult]:
    ...
    for case in GROUNDEDNESS_CASES:
        for run in range(1, runs + 1):
            ...
            try:
                answer = present_node(state)["messages"][-1].text
...
# ---------------------------------------------------- suite: compliance --
# Tests the guardrail's JUDGE with FAKE_RULES in hand (no DB): does the
# compliance node revise violations and leave compliant answers alone?


def run_compliance_suite(llm, runs: int) -> list[RunResult]:
    node = make_compliance_node(llm, rule_retriever=lambda query: FAKE_RULES)
    results = []
        ...
        for run in range(1, runs + 1):
            state = {
                "messages": [
                    ...
                    AIMessage(content=case["draft"], id="draft-1"),
                ]
            }
            try:
                out = node(state)
            except Exception as exc:
                results.append(RunResult("compliance", case["name"], run, False, f"exception: {exc}"))
                continue

            status = out.get("compliance_status")
            # A verdict of None means the judge errored (fail-open) — treat
            # as a failed run so infra problems surface, not silently pass.
            passed = status == case["expect"]
            detail = "" if passed else f"expected {case['expect']!r}, got compliance_status={status!r}"
            results.append(RunResult("compliance", case["name"], run, passed, detail))
    return results


# ----------------------------------------------------- suite: retrieval --
# Tests the guardrail's RETRIEVAL: does the vector search surface the rule
# that actually applies? Needs the ingested rulebook — skips if absent.


def run_retrieval_suite() -> list[RunResult]:
    from compliance.retriever import build_rule_retriever

    retriever = build_rule_retriever(k=4)
    if retriever is None:
        print(
            "\n=== retrieval ===\n  SKIPPED — no rules ingested (run "
            "compliance/generate_rules_pdf.py then compliance/ingest_rules.py), "
            "or the DB/Ollama is unreachable."
        )
        return []
    ...
    for case in RETRIEVAL_CASES:
        try:
            rules = retriever(case["query"])
            sections = [r["section"] for r in rules]
        except Exception as exc:
            results.append(RunResult("retrieval", case["name"], 1, False, f"exception: {exc}"))
            continue
        passed = case["expect_section"] in sections
        detail = "" if passed else f"expected {case['expect_section']} in top-4; got {sections}"
        results.append(RunResult("retrieval", case["name"], 1, passed, detail))
    return results
...
def report(results: list[RunResult]) -> bool:
    ...
    for (suite, case), runs_list in by_case.items():
        if suite != current_suite:
            current_suite = suite
            print(f"\n=== {suite} ===")
        n_pass = sum(r.passed for r in runs_list)
        n = len(runs_list)
...
def build_llm(role: str):
    # Same factory as api/setup.py (api/llm.py, CHAT_MODEL_FAST/SMART or
    # CHAT_MODEL in .env) so evals always test the provider the app
    # actually runs — but at temperature 0, the most deterministic
    # behavior the model can give. Bake-off trick: run the suite twice
    # with different CHAT_MODEL values and compare the two score tables.
    from llm import build_chat_model

    return build_chat_model(role=role, temperature=0.0)


SUITES = ("confidence", "intake", "routing", "groundedness", "compliance", "retrieval")
...
def main() -> int:
    ...
    # Deterministic / no-LLM suites first.
    if "confidence" in selected:
        results += run_confidence_suite()
    if "retrieval" in selected:
        results += run_retrieval_suite()  # uses embeddings, not a chat LLM

    llm_suites = [s for s in selected if s in ("intake", "routing", "groundedness", "compliance")]
    if llm_suites:
        fast_llm = build_llm("fast")
        smart_llm = build_llm("smart")
        if "intake" in llm_suites:
            results += run_intake_suite(fast_llm, args.runs)
        if "routing" in llm_suites:
            results += run_routing_suite(fast_llm, smart_llm, args.runs)
        if "groundedness" in llm_suites:
            results += run_groundedness_suite(smart_llm, args.runs)
        if "compliance" in llm_suites:
            results += run_compliance_suite(smart_llm, args.runs)
