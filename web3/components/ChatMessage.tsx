    ...
    <div className={`flex flex-col ${isUser ? "items-end" : "items-start"} gap-2`}>
      ...
      {(isUser || hasContent) && (
        <div className="flex items-end gap-2 max-w-[85%] sm:max-w-[70%]">
          ...
          <div
            className={
              isUser
                ? "rounded-2xl rounded-br-sm bg-verdigris/20 border border-verdigris/40 px-4 py-2.5 text-parchment"
                : "rounded-2xl rounded-bl-sm bg-ink-2 border border-rule px-4 py-2.5 text-parchment"
            }
          >
      ...
      {!isUser && typeof message.confidenceScore === "number" && (
        <div className="ml-0 sm:ml-4 opacity-90">
          <CompassGauge score={message.confidenceScore} size={40} />
        </div>
      )}
